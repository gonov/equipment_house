...
import {jwtDecode} from 'jwt-decode'; // Исправлено: Импорт должен быть без фигурных скобок
...
function Header() {
  ...
  const updateUserData = () => {
    try {
      ...
      if (!token) {
        setUserData(null); // Если токен отсутствует
        return;
      }
  ...
  // Обновляем состояние при загрузке компонента и изменении токена
  useEffect(() => {
    updateUserData();
    ...
    // Подписка на изменения localStorage и cookies
