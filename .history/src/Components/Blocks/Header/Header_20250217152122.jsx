...
function Header() {
  ...
  useEffect(() => {
    // Получаем данные из localStorage (или другого хранилища)
              ...
              {userData ? (
                <>
                  <button onClick={() => navigate('/basket')}>
                    ...
                    <span>Корзина     {cartCount > 0 && (
                      <span className={classes.cartBadge}>{cartCount}123</span>
                    )}</span>
            ...
            <ul>
              {categories?.slice(0, 5).map((el) => (
                <li
                  ...
                  onClick={() => (window.location.href = `/category/${el.id}`)}
