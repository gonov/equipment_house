...
function Header() {
  ...
  useEffect(() => {
    ...
    if (cartUpdated) {
      fetchCartCount();
      setCartUpdated(false); // Сбрасываем флаг
    }
  }, [cartUpdated]); // Перезапускать, когда cartUpdated изменится

  const handleAddToCart = () => {
    // Логика добавления товара в корзину
    setCartUpdated(true); // Устанавливаем флаг на обновление корзины
  };
  ...
  useEffect(() => {
    const fetchCartCount = async () => {
      try {
        ...
        const response = await fetch(`${serverConfig}/api/cart/count`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    fetchCartCount();
  }, [userData]); // Обновление корзины при изменении userData
