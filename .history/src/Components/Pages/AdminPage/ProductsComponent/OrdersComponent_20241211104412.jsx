      ...
      <ArrayInput source="orderItems">
        <SimpleFormIterator>
          ...
          <DeleteButton 
            label="Удалить товар" 
            onClick={async () => {
              // Удаление товара из массива на фронте
              const updatedItems = props.record.orderItems.filter(item => item.productId !== props.record.productId);
              // Обновление данных в форме, отправка данных на сервер
              // (сделайте соответствующие изменения для отправки данных через API)
            }} 
          />
        </SimpleFormIterator>
      </ArrayInput>
