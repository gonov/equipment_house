...
import atob from 'atob'; // Библиотека для декодирования из Base64
...
function BasketPage() {
  ...
  // Извлекаем данные пользователя из куки (предположим, что они закодированы в Base64)
  const name = Cookies.get('name') ? atob(Cookies.get('name')) : ''; // Декодирование имени
  const email = Cookies.get('email') ? atob(Cookies.get('email')) : ''; // Декодирование почты
