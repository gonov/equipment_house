      ...
      <div className={classes.card} onClick={goToProductPage}>
        <div className={classes.imageContainer}>
          <img
            src={
              product.img[0]
                ? product.img[0].startsWith('http') // проверяем, если это полный путь (например, из XML)
                  ? product.img[0] // если да, используем его
                  : `${uploadsConfig}${product.img[0]}` // если нет, то добавляем путь к папке uploads
                : `${uploadsConfig}/default-image.jpg` // если изображения нет, используем дефолтное
            }
            alt={product.name}
          />
        </div>
