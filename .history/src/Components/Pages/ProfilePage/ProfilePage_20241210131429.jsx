  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('Orders Data:', ordersData); // Проверяем данные в консоли

        // Проверяем, есть ли поле "date" у каждого заказа
        const ordersWithValidDate = ordersData.map((order) => {
          if (!order.date) {
            console.error('No date found for order', order);
            return { ...order, date: '1970-01-01' }; // Если нет даты, ставим дефолтную
          }
          return order;
        });

        // Сортируем заказы по дате, чтобы свежие были в начале
        const sortedOrders = ordersWithValidDate.sort((a, b) => new Date(b.date) - new Date(a.date));
