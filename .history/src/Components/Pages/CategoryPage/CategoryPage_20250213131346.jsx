...
function CategoryPage({ children, ...props }) {
  ...
  function CategoryPage() {
    const { id } = useParams(); // Получаем categoryId из URL
    const categoryId = parseInt(id); // Приводим к числу
    const [selectedCategoryId, setSelectedCategoryId] = useState(categoryId);

    useEffect(() => {
      setSelectedCategoryId(categoryId); // Обновляем при изменении URL
    }, [categoryId]);
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        try {
          ...
          const productsResponse = await axios.get(
            `${serverConfig}/products?categoryId=${selectedCategoryId}`
          );
      ...
      fetchData();
    }, [selectedCategoryId]); // Загружаем товары при изменении selectedCategoryId
  }
