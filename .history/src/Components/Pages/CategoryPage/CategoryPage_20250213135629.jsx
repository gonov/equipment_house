...
function CategoryPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${serverConfig}/products?categoryId=${id}`);
    ...
    if (id) {
      fetchProducts();
      setCurrentPage(0); // Сбрасываем пагинацию при смене категории
    }
  }, [id]); // Перезагрузка товаров при изменении категории
    ...
    <CenterBlock>
      <WidthBlock>
        <div className={classes.categoryName}>
          <span>{categories.find((c) => c.id === parseInt(id))?.title || 'Категория не найдена'}</span>
        </div>
          ...
          <div className={classes.leftBlock}>
            ...
            <div className={classes.categoryList}>
              {categories.map((category) => (
                <div
                  key={category.id}
                  className={`${classes.leftBlockCategory} ${category.id === parseInt(id) ? classes.activeCategory : ''}`}
      ...
      {/* Всплывающее сообщение Material-UI */}
