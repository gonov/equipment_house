...
function CategoryPage() {
  ...
  const location = useLocation(); // Следим за изменениями URL
  ...
  // Загружаем категории и товары при изменении `categoryId`
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const productsResponse = await axios.get(`${serverConfig}/products?categoryId=${categoryId}`);
        setProducts(productsResponse.data); // Присваиваем только товары текущей категории
      } catch (err) {
    ...
    fetchData();
  }, [categoryId, location.key]); // Теперь useEffect обновляется при изменении категории
    ...
    <CenterBlock>
      <WidthBlock>
        <div className={classes.categoryName}>
          <span>
            {categories.find((c) => c.id === categoryId)?.title || 'Категория не найдена'}
          </span>
        </div>
          ...
          <div className={classes.leftBlock}>
            ...
            <div className={classes.categoryList}>
              {categories.map((category) => (
                <div
                  key={category.id}
                  className={`${classes.leftBlockCategory} ${category.id === categoryId ? classes.activeCategory : ''}`}
